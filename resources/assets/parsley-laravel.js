...
window.ParsleyValidator
    .addValidator('same', function (value, requirement) {
        return $(requirement).val() == value;
    }, 32)
    .addMessage('en', 'same', 'The value should be the same as "%s"');
window.ParsleyValidator
    .addValidator('activeUrl', function (value, requirement) {
        $.ajax({
            url: $(requirement).val(),
            complete: function(request, status)
            {
                if(status != 'success' && status != 'notmodified' && status != 'nocontent')
                {
                    window.ParsleyUI.addError($(requirement).parsley(), 'activeUrl', 'The provided url is not live');
                }
                else
                {
                    window.ParsleyUI.removeError($(requirement).parsley(), 'activeUrl');
                }
            }
        });
    }, 32)
    .addMessage('en', 'activeUrl', 'The provided url "%s" is not live');